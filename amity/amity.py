...
class Amity(object):
    ...
    def add_person(self, position, person_name, want_accomodation=None):
        ...
        if person_name in self.all_persons['staff'] + self.all_persons['fellow']:
    ...
    def random_living_space_space(self, person_name):
        ...
        if len(self.all_rooms['office']) > 0:
            ...
            room_name = [room_name for room_name in self.all_rooms['living_space'] if room_name not in self.all_allocations['living_space'].keys()]
            available_room.extend(room_name)
    ...
    def reallocate_fellow(self, person_name, new_room_name):
        ...
        if new_room_name in self.all_rooms['office']:
            if new_room_name in self.all_allocations['office'].keys() and len(
                ...
                # Reallocate to the new room
                self.all_allocations['office'][new_room_name].append(person_name)
                cprint('%s has been reallocated successfully...' % person_name, 'white')
            elif new_room_name in self.all_allocations['office'].keys() and len(
                    self.all_allocations['office'][new_room_name]) > 6:
    ...
    def reallocate_person(self, person_name, new_room_name):
        ...
        """
        if person_name in self.all_persons['staff'] and new_room_name in self.all_rooms['office']:
        ...
        elif person_name in self.all_persons['staff'] and new_room_name in self.all_rooms['living_space']:
            cprint('Staff can not be reallocated to a living space', 'red')
        # Checks if person is in the system
        elif person_name not in self.all_persons['staff'] and person_name not in self.all_persons['fellow']:
    ...
    def check_empty_offices(self, key):
        ...
        if len(self.all_allocations['office'][key]) == 0:
            cprint('%s has no allocations at the moment...', 'white')
        else:
            pass
    ...
    def load_people(self, filename):
        ...
        try:
            ...
            print('That If file does not exist... Please try again.')
    ...
    def load_state(self, db_name=None):
        ...
        try:
            ...
            try:
                ...
                cprint('There are no Unallocated people in the Database', 'white')
        except:
        ...
        try:
            ...
            if filename:
                ...
                if self.all_rooms['living_space']:
                    ...
                    cprint("There are no living_space at the moment...", 'white')
        except:
            cprint('An error occurred.', 'red')
